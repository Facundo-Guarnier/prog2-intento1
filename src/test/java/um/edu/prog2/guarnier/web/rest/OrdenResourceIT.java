...
class OrdenResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/ordens";
    ...
    @Autowired
    private OrdenSearchRepository ordenSearchRepository;
    ...
    public void cleanupElasticSearchRepository() {
        ordenSearchRepository.deleteAll();
        assertThat(ordenSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createOrden() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(ordenSearchRepository.findAll());
        // Create the Orden
        restOrdenMockMvc
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(ordenSearchRepository.findAll());
    ...
    void putExistingOrden() throws Exception {
        ...
        ordenSearchRepository.save(orden);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(ordenSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(ordenSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<Orden> ordenSearchList = IterableUtils.toList(ordenSearchRepository.findAll());
                Orden testOrdenSearch = ordenSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testOrdenSearch.getCliente()).isEqualTo(UPDATED_CLIENTE);
                assertThat(testOrdenSearch.getAccionId()).isEqualTo(UPDATED_ACCION_ID);
                assertThat(testOrdenSearch.getAccion()).isEqualTo(UPDATED_ACCION);
                assertThat(testOrdenSearch.getOperacion()).isEqualTo(UPDATED_OPERACION);
                assertThat(testOrdenSearch.getPrecio()).isEqualTo(UPDATED_PRECIO);
                assertThat(testOrdenSearch.getCantidad()).isEqualTo(UPDATED_CANTIDAD);
                assertThat(testOrdenSearch.getFechaOperacion()).isEqualTo(UPDATED_FECHA_OPERACION);
                assertThat(testOrdenSearch.getModo()).isEqualTo(UPDATED_MODO);
                assertThat(testOrdenSearch.getEstado()).isEqualTo(UPDATED_ESTADO);
            });
    }
    ...
    @Test
    @Transactional
    void searchOrden() throws Exception {
        // Initialize the database
        ...
        // Search the orden
        restOrdenMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + orden.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(orden.getId().intValue())))
            .andExpect(jsonPath("$.[*].cliente").value(hasItem(DEFAULT_CLIENTE)))
